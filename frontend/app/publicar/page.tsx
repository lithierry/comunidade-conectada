...
export default function PublishPage() {
  ...
  return <div className="page container"><div className={styles.intro}><p className="eyebrow">Novo anúncio</p><h1>Compartilhe com a comunidade</h1></div>{created ? <div className={styles.success}><Notice kind="success" title="Publicação disponível">Seu anúncio já está visível no mural.</Notice><Link className="button primary" href={`/anuncio/${created.id}`}>Ver publicação</Link><Link className="button secondary" href="/minhas">Minhas publicações</Link></div> : <AnnouncementForm submitLabel="Publicar anúncio" onSubmit={submit} submitting={saving} />}</div>;
}
