...
export function ProfileCompletionForm({ initialName, onSuccess }: { initialName: string; onSuccess: () => void | Promise<void> }) {
  ...
  return <form className={styles.form} onSubmit={submit} noValidate>
    ...
    <p>Vincule seus dados antes de criar ou alterar publicações.</p>
    <div className="form-field">
      <label htmlFor="complete-name">Nome completo</label>
      <input id="complete-name" type="text" autoComplete="name" minLength={2} maxLength={100} required value={name} onChange={(event) => setName(event.target.value)} aria-describedby="complete-name-help" />
      <small id="complete-name-help">Este nome identifica sua conta e não é publicado automaticamente nos anúncios.</small>
    </div>
